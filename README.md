# Rescue-AI

RescueAI is a multi-agent autonomous disaster response system that uses AI agents, data analytics  to analyze disaster situations and make real-time decisions on priority zones, resource allocation, and emergency alerts. It turns hours of manual disaster management work into 3 seconds of automated AI-driven action.


screenshots
<img width="1916" height="867" alt="image" src="https://github.com/user-attachments/assets/5afe1cd2-2b2e-4f87-87fb-d0853c93c466" />







<img width="1917" height="868" alt="image" src="https://github.com/user-attachments/assets/ac027654-6ebd-437c-bb62-f9cf4914f477" />











<img width="1918" height="852" alt="image" src="https://github.com/user-attachments/assets/5279de6e-5a8b-4294-a54e-c529a4d0226a" />


